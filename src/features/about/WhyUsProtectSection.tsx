...
function Card({ card }: { card: ProtectCard }) {
  return (
    <div className="relative isolate flex min-h-[220px] flex-col justify-between overflow-hidden rounded-xl p-6 text-white">
      <img
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="font-stat text-[32px] font-bold leading-none text-white/30">
          {card.index}
        </span>
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <ul className="flex flex-col gap-1.5">
          {card.bullets.map((bullet, index) => (
            <li key={index} className="flex items-start gap-2 text-[14px] font-medium text-white/90">
